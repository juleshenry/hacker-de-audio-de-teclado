...
# Arquitetura do Modelo (estilo simplificado CoAtNet/ResNet para Mel-Espectrogramas)
...
def extract_mel_spectrogram(audio_path, sr=44100):
    ...
    # Obter os onsets (início das teclas)
    onsets = librosa.onset.onset_detect(y=y, sr=sr, backtrack=True)

    spectrograms = []
    # Extrair blocos de 300ms em torno de cada onset
...
def predict_audio(
    audio_path, model_path="keystroke_model.pth", classes_path="classes.txt"
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--treinar",
        type=str,
        help="Diretório contendo arquivos de áudio de treino organizados por classe (ex: dados/a/, dados/b/, ...)",
    )
    parser.add_argument(
    ...
    if args.treinar:
        print(f"Treinando o modelo com dados de {args.treinar}...")
        train_model(args.treinar)
    elif args.prever:
        print(f"Prevendo teclas para {args.prever}...")
        predict_audio(args.prever)
    else:
        print(
            "Por favor, especifique --treinar <dir_dados> ou --prever <arquivo_audio>"
        )
        print("Exemplo 1: python teclahacker.py --treinar dados_meu_teclado/")
        print("Exemplo 2: python teclahacker.py --prever o_zorro_e_gris.wav")
